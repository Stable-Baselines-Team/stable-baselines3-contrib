...
class ARS(BaseAlgorithm):
    ...
    def _collect_rollouts(self, deltas, callback):
        batch_steps = 0
        theta_idx = 0
    ...
    def learn(
        ...
        while self.num_timesteps < total_steps:
            ...
            deltas = self.rng.standard_normal((self.n_delta, self.n_params)) * sigma_this_step

            candidate_returns, batch_steps = self._collect_rollouts(deltas, callback)
