...
class TQC(OffPolicyAlgorithm):
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            if self.num_timesteps > learning_starts and True:
                ...
                with th.no_grad():
                    quantiles = self.critic(
                        ...
                        self.replay_buffer.to_torch(actions, copy=False),
                    )
                    # shape: (n_envs, n_critics)
                    aleatoric_uncertainty = th.var(quantiles, dim=2).max().item()
                    # shape: (n_envs, n_critics)
                    critics_mean = th.mean(quantiles, dim=2)
                    # shape: (n_envs, 1)
            ...
            # If too certain, truncate
            for env_idx in no_uncertainty_env_indices:
                if not dones[env_idx]:
                    assert isinstance(self.env.unwrapped, DummyVecEnv)
                    vec_env = self.env.unwrapped
                    dones[env_idx] = True
                    infos[env_idx]["TimeLimit.truncated"] = True
                    infos[env_idx]["terminal_observation"] = new_obs[env_idx].copy()
                    new_obs_single_env, _ = vec_env.envs[env_idx].reset()
                    # TODO: update monitor to have the right stats
                    vec_env._save_obs(env_idx, new_obs_single_env)
                    new_obs[env_idx] = new_obs_single_env
