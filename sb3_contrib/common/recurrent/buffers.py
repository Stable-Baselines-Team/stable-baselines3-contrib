...
from sb3_contrib.common.recurrent.type_aliases import (
    RecurrentSequenceRolloutBufferSamples,
...
class RecurrentSequenceRolloutBuffer(RecurrentRolloutBuffer):
    ...
    def reset(self) -> None:
        super().reset()
        self.observations = [None for _ in range(self.buffer_size)]
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RecurrentDictRolloutBufferSamples, None, None]:
        ...
        if not self.generator_ready:
            ...
            self.len_step = np.array([o.shape[1] for o in self.observations] * self.n_envs)
            self.end_step = self.len_step.cumsum() - 1
            self.start_step = np.insert(self.end_step[:-1] + 1, 0, 0)
            self.observations = np.concatenate(self.observations, axis=1)
            shape = self.observations.shape
            self.observations = self.observations.reshape(shape[0] * shape[1], *shape[2:])
    ...
    def _get_samples(
        ...
        observations = pad(
            self.start_step[batch_inds],
            self.end_step[batch_inds],
            self.device,
            self.observations
        )
        observations = self.pad(observations)

        return RecurrentSequenceRolloutBufferSamples(
            observations=observations.reshape((padded_batch_size,) + observations.shape[2:]),
            actions=self.pad(self.actions[batch_inds]).reshape((padded_batch_size,) + self.actions.shape[1:]),
            old_values=self.pad_and_flatten(self.values[batch_inds]),
            old_log_prob=self.pad_and_flatten(self.log_probs[batch_inds]),
            advantages=self.pad_and_flatten(self.advantages[batch_inds]),
            ...
            episode_starts=self.pad_and_flatten(self.episode_starts[batch_inds]),
            mask=self.pad_and_flatten(np.ones_like(self.returns[batch_inds])),
            step_ends=self.pad_and_flatten(self.len_step[batch_inds] - 1)
        )
