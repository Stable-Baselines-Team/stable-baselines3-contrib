...
class AttentionActorCriticPolicy(ActorCriticPolicy):
    ...
    def _process_sequence(
        ...
        """
        #print(attn_memory.size(), features.size())
        # LSTM logic
    ...
    def forward(
        ...
        #latent_pi, attn_states_pi = self._process_sequence(pi_features, attn_states.pi, episode_starts, self.lstm_actor)
        latent_pi, attn_memory_pi = self._process_sequence(pi_features, attn_memory.pi, episode_starts, self.model)
        # if self.lstm_critic is not None:
        #     latent_vf, attn_states_vf = self._process_sequence(vf_features, attn_states.vf, episode_starts, self.lstm_critic)
        # elif self.shared_lstm:
        #     # Re-use LSTM features but do not backpropagate
        #     latent_vf = latent_pi.detach()
        #     attn_states_vf = (attn_states_pi[0].detach(), attn_states_pi[1].detach())
        # else:
        #     # Critic only has a feedforward network
        #     latent_vf = self.critic(vf_features)
        #     attn_states_vf = attn_states_pi
        #print('out:', latent_pi.size(), 'attn_memory:', attn_states_pi.size())
        # attn_states_vf = attn_states_pi.detach()
        latent_vf = latent_pi.detach()
    ...
    def get_distribution(
        ...
        :param attn_states: The last hidden and memory states for the LSTM.
        :param episode_starts: Whether the observations correspond to new episodes
            or not (we reset the lstm states in that case).
        ...
        #latent_pi, attn_states = self._process_sequence(features, attn_states, episode_starts, self.lstm_actor)
    ...
    def predict_values(
        ...
        # if self.lstm_critic is not None:
        #     latent_vf, attn_states = self._process_sequence(features, attn_states, episode_starts, self.lstm_critic)
        # elif self.shared_lstm:
        #     # Use LSTM from the actor
        #     latent_pi, _ = self._process_sequence(features, attn_states, episode_starts, self.lstm_actor)
        #     latent_vf = latent_pi.detach()
        # else:
    ...
    def evaluate_actions(
        ...
        # if self.lstm_critic is not None:
        #     latent_vf, _ = self._process_sequence(vf_features, attn_states.vf, episode_starts, self.lstm_critic)
        # elif self.shared_lstm:
        #     latent_vf = latent_pi.detach()
        # else:
        #     latent_vf = self.critic(vf_features)
    ...
    def _predict(
        ...
        :return: Taken action according to the policy and hidden states of the Attention network
        """
    ...
    def predict(
        ...
        with th.no_grad():
            ...
            actions, memory = self._predict(
                observation, attn_states=memory, episode_starts=episode_starts, deterministic=deterministic
            )
...
class AttentionActorCriticCnnPolicy(AttentionActorCriticPolicy):
    ...
    :param n_lstm_layers: Number of LSTM layers.
    :param shared_lstm: Whether the LSTM is shared between the actor and the critic.
        By default, only the actor has a attention network.
    :param enable_critic_lstm: Use a seperate LSTM for the critic.
    :param lstm_kwargs: Additional keyword arguments to pass the the LSTM
        constructor.
    """
