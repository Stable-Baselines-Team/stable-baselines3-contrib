...
from sb3_contrib.common.attention.type_aliases import (
    AttentionDictRolloutBufferSamples,
    AttentionRolloutBufferSamples,
    AttnMemory,
)
...
class AttentionRolloutBuffer(RolloutBuffer):
    """
    Rollout buffer that also stores the memory for attention network (GTrXL).

    :param buffer_size: Max number of element in the buffer
    :param observation_space: Observation space
    :param action_space: Action space
    :param memory_shape: Shape of the buffer that will collect the attention memory
        (n_steps, num_layers, n_envs, attention_dim)
    :param device: PyTorch device
    ...
    def reset(self):
        super().reset()
        self.memory_pi = np.zeros(self.memory_shape, dtype=np.float32)
        # self.cell_states_pi = np.zeros(self.memory_shape, dtype=np.float32)
        # self.memory_vf = np.zeros(self.memory_shape, dtype=np.float32)
        # self.cell_states_vf = np.zeros(self.memory_shape, dtype=np.float32)

    def add(self, *args, attn_memory: AttnMemory, **kwargs) -> None:
        """
        :param attn_memory: Attention memory
        """
        self.memory_pi[self.pos] = np.array(attn_memory.pi.cpu().numpy())
        # self.cell_states_pi[self.pos] = np.array(attn_memory.pi[1].cpu().numpy())
        # self.memory_vf[self.pos] = np.array(attn_memory.vf[0].cpu().numpy())
        # self.cell_states_vf[self.pos] = np.array(attn_memory.vf[1].cpu().numpy())

        super().add(*args, **kwargs)

    def get(self, batch_size: Optional[int] = None) -> Generator[AttentionRolloutBufferSamples, None, None]:
        assert self.full, "Rollout buffer must be full before sampling from it"

        # Prepare the data
        if not self.generator_ready:
            # memory_shape = (n_steps, num_layers, n_envs, attention_dim)
            # swap first to (n_steps, n_envs, num_layers, attention_dim)
            for tensor in ["memory_pi"]:#, "cell_states_pi"]:#, "memory_vf", "cell_states_vf"]:
    ...
    def _get_samples(
        ...
        # print('n_seq', n_seq, 'max_length', max_length, 'buffer_size', self.buffer_size, 'batch_inds', len(batch_inds))
        padded_batch_size = n_seq * max_length
        # Retrieving attention memory that will allow
        # to properly initialize the GTrXL at the beginning of each sequence
        # attn_memory_pi = (
            ...
            # self.memory_pi[batch_inds][self.seq_start_indices].swapaxes(0, 1),
            # self.cell_states_pi[batch_inds][self.seq_start_indices].swapaxes(0, 1),
        # )

        # print(self.memory_pi.shape, batch_inds, self.seq_start_indices)
        attn_memory_pi = self.to_torch(self.memory_pi[batch_inds][self.seq_start_indices].swapaxes(0, 1)).contiguous()
        # attn_memory_vf = (
        #     # (n_envs * n_steps, n_layers, dim) -> (n_layers, n_seq, dim)
        #     self.memory_vf[batch_inds][self.seq_start_indices].swapaxes(0, 1),
        #     self.cell_states_vf[batch_inds][self.seq_start_indices].swapaxes(0, 1),
        # )
        # attn_memory_pi = (self.to_torch(attn_memory_pi[0]).contiguous(), self.to_torch(attn_memory_pi[1]).contiguous())
        # attn_memory_vf = (self.to_torch(attn_memory_vf[0]).contiguous(), self.to_torch(attn_memory_vf[1]).contiguous())
        # print('padded_batch_size', padded_batch_size, 'obs', self.obs_shape)
        # print('attn_memory_pi', attn_memory_pi.size())
        return AttentionRolloutBufferSamples(
            ...
            attn_memory=AttnMemory(attn_memory_pi),
            episode_starts=self.pad_and_flatten(self.episode_starts[batch_inds]),
            mask=self.pad_and_flatten(np.ones_like(self.returns[batch_inds])),
        )


class AttentionDictRolloutBuffer(DictRolloutBuffer):
    """
    Dict Rollout buffer used in on-policy algorithms like A2C/PPO.
    Extends the AttentionRolloutBuffer to use dictionary observations

    :param buffer_size: Max number of element in the buffer
    :param observation_space: Observation space
    :param action_space: Action space
    :param memory_shape: Shape of the buffer that will collect memory
    :param device: PyTorch device
    :param gae_lambda: Factor for trade-off of bias vs variance for Generalized Advantage Estimator
        Equivalent to classic advantage when set to 1.
    :param gamma: Discount factor
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[AttentionDictRolloutBufferSamples, None, None]:
        assert self.full, "Rollout buffer must be full before sampling from it"
    ...
    def _get_samples(
        ...
        # attn_memory_pi = (
            ...
            # self.cell_states_pi[batch_inds][self.seq_start_indices].swapaxes(0, 1),
        # )
        attn_memory_pi = self.memory_pi[batch_inds][self.seq_start_indices].swapaxes(0, 1)
        # attn_memory_vf = (
        #     # (n_envs * n_steps, n_layers, dim) -> (n_layers, n_seq, dim)
        #     self.memory_vf[batch_inds][self.seq_start_indices].swapaxes(0, 1),
        #     self.cell_states_vf[batch_inds][self.seq_start_indices].swapaxes(0, 1),
        ...
        return AttentionDictRolloutBufferSamples(
