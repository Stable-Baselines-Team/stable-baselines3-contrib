...
class FeatureExtractor(nn.Module):
    ...
    def __init__(
        ...
        layers.append(nn.Linear(last_layer_dim, feature_dim))
...
class Goalify(gym.GoalEnv, gym.Wrapper):
    """
    Make an envrionment goal-conditionned.

    The goal is to reach a specific latent goal.
    ``reward`` is :math:`r = \log q_\varphi(z | s_{t+1}) - log p(z)` where :math:`q_\varphi` is
    ...
    :param skills: The skills (from latent space)
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "desired_goal": spaces.Box(-10, 10, (self.nb_skills,)),
            }
        )
    ...
    def reset(self) -> Dict[str, np.ndarray]:
        self.skill = self.skill_distribution.sample()
    ...
    def _get_dict_obs(self, obs: np.ndarray) -> Dict[str, np.ndarray]:
        desired_goal = F.one_hot(self.skill, num_classes=self.nb_skills)
        return {
            "observation": obs,
...
class DIAYN(SAC):
    def __init__(
        ...
        self.discriminator = Discriminator(obs_dim, net_arch=[256, 256], nb_skills=nb_skills)
        env = Goalify(env, skill_distribution, self.discriminator)
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            observations = replay_data.observations["observation"]
            input = self.discriminator.probs(observations)
            target = replay_data.observations["desired_goal"]
